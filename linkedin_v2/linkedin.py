...
class LinkedInApplication(object):
    ...
    def find_member_organization_access_info(self, **kwargs):
        ...
        url = \
            "%s/organizationalEntityAcls?q=roleAssignee&role=ADMINISTRATOR&projection=(elements*(*,roleAssignee~(localizedFirstName,localizedLastName),organizationalTarget~(id,name,localizedName,vanityName,logoV2(original~:playableStreams))))" % ENDPOINTS.BASE
        return self.make_get_request(url)
